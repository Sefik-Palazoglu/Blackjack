...
Game::Game(int deckNumber, Player& player, Player& dealer)
	: m_deckNumber{ deckNumber }, m_deck{ Deck{deckNumber} }, m_player{ player }, m_dealer{ dealer }
{
...
Game::Result Game::playBlackjack()
{
	...
	if (m_player.score() < m_dealer.score())		return Game::Result::DEALER_WIN;
	else if (m_dealer.score() < m_player.score())	return Game::Result::PLAYER_WIN;
	else											return Game::Result::TIE;
}
...
bool Game::dealerTurn()
{
	...
	return m_dealer.isBust(); // return true if dealer went bust; otherwise, false.
}
